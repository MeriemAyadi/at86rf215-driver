...
static void at86rf215_irq_trx_end(void *context)
{
	...
	if (lp->is_tx) {
		lp->is_tx = 0;
		/* WHY do we write the RG_RF09_STATE adress on the SPI bus and then we don't read the register value ???? */
		at86rf215_async_read_reg(lp, RG_RF09_STATE, ctx, at86rf215_tx_trac_check);
	} else {
		at86rf215_async_read_reg(lp, RG_RF09_STATE, ctx, at86rf215_rx_trac_check);
	}
}
...
static irqreturn_t at86rf215_isr(int irq, void *data)
{
	...
	/* Disables the interrupt associated with "irq" without waiting for any currently executing instances ofthe interrupt handler to return */
...
/* This function is called by at86rf215_xmit and is used to determine transitions between the following states : STATE_TX_ARET_ON, STATE_TX_ON, STATE_TRX_OFF. */
static void at86rf215_xmit_start(void *context){}
/* This is the handler that 802.15.4 module calls for each frame in the skb going to be transmitted through the hardware device. */
...
/* This is the handler that 802.15.4 module calls for the hardware device initialization ( to be ready to receive data ). */
...
/* This is the handler that 802.15.4 module calls for the hardware device cleanup (after reception). */
...
{
        int rc, irq_type, irq_pol = IRQ_ACTIVE_HIGH; /* Elle peut egalement etre configuree en IRQ_ACTIVE_LOW */
        //u8 csma_seed[2];
...
/* NOT YET, this is not important for now.
        ...
        if (rc)
                return rc;
*/
        /* CLKM changes are applied immediately (In case thera are CLKM clock rate modifications)*/
...
/* Please check cfg802154.h */
        lp->hw->phy->flags = WPAN_PHY_FLAG_TXPOWER |  /* Indicates that transceiver will support transmit power setting (du signal) */
...
/* (CCA-ED) is supported only , please refer to Datasheet : page 148 */
	...
	lp->hw->phy->current_page = 0;
        ...
        lp->hw->phy->symbol_duration = 4; /* at86rf215 (ttx_start_delay)  , at86rf230 (tTR10 ) ==> c'est la duree d'un symbole PSDU module et code */
...
static int at86rf215_probe(struct spi_device *spi)
{
	...
	/* The followinf function return the address of the pointer lp->hw */
	...
	/* This function initialize a dynamically allocated completion pointer "lp->state_complete" to completion structure that is to be initialized */
        ...
        /* Read irq status register to reset irq line ( this is akward, we didnt use status later ) */
